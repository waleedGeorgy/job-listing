...
const AddJobPage = () => {
    ...
    return (
        <div className="max-w-xl mx-auto flex flex-col">
            ...
            <form className="space-y-5" action={(formData: FormData) => startTransition(() => postJobWithResponse(formData))}>
                <div className="flex flex-col gap-2">
                    <div className="flex flex-row items-center justify-between">
                        <label htmlFor="title" className="text-sm font-semibold opacity-60">Job Title*</label>
                        {state?.title && (<span className="text-xs text-red-400">{state.title}</span>)}
                    </div>
                ...
                <div className="flex flex-col gap-2">
                    <div className="flex flex-row items-center justify-between">
                        ...
                        {state?.company && (<span className="text-xs text-red-400">{state.company}</span>)}
                    </div>
                ...
                <div className="flex flex-col gap-2">
                    <div className="flex flex-row items-center justify-between">
                        ...
                        {state?.location && (<span className="text-xs text-red-400">{state.location}</span>)}
                    </div>
                ...
                <div className="flex flex-col gap-2">
                    <div className="flex flex-row items-center justify-between">
                        ...
                        {state?.description && (<span className="text-xs text-red-400">{state.description}</span>)}
                    </div>
