    ...
    <div className="flex flex-col gap-3 mt-12 max-w-4xl mx-auto px-2 text-center">
      ...
      {session?.user ?
        <p>
          ...
          <Link
            href="/jobs"
            className="text-blue-500 underline underline-offset-2 hover:text-blue-400 transition-colors duration-200"
          >
            browse jobs
          </Link>
          ,{" "}
          ...
          ,{" "}or check that status of all your application in the{" "}
          <Link
            href="/dashboard"
            className="text-blue-500 underline underline-offset-2 hover:text-blue-400 transition-colors duration-200"
          >
        ...
        {recentJobs.map(recentJob => (
          <div key={recentJob?.id} className="px-6 py-4 flex flex-col gap-1.5 bg-gray-800 rounded-lg w-full hover:outline-2 hover:outline-emerald-600">
            <div className="flex flex-row items-center justify-between gap-1 w-full">
              <p className="font-roboto font-bold text-lg line-clamp-1">{recentJob.title}</p>
            ...
            <p className="text-left text-sm my-3 line-clamp-2">{recentJob.description}</p>
